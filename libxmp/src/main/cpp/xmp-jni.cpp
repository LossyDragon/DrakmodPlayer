...
JNI_FUNCTION(init)(JNIEnv *env, jobject obj, jint rate, jint ms, jint mode, jint channels,
                   jint api, jint flags) {
...
JNI_FUNCTION(loadModuleFd)(JNIEnv *env, jobject obj, jint fd, jobject modInfo) {
    ...
    XmpPlayerState &state = XmpPlayerState::instance();
    res = xmp_load_module_from_file(state.getContext(), file.get(),
                                    static_cast<off_t>(statbuf.st_size));
...
JNI_FUNCTION(testModuleFd)(JNIEnv *env, jobject obj, jint fd, jobject modInfo) {
    ...
    if (res == 0) {
        ...
        char path[PATH_MAX];
        ssize_t len = readlink(("/proc/self/fd/" + std::to_string(fd)).c_str(), path,
                               sizeof(path) - 1);
        if (len > 0) path[len] = '\0';
        LOG_WARN("testModuleFd() - not a valid module, error: %d, file: %s", res,
                 len > 0 ? path : "unknown");
    }
...
JNIEXPORT jint JNICALL
JNI_FUNCTION(setPlayerNative)(JNIEnv *env, jobject obj, jint parm, jint value) {
    XmpPlayerState &state = XmpPlayerState::instance();
    return xmp_set_player(state.getContext(), parm, value);
}
...
JNIEXPORT void JNICALL JNI_FUNCTION(getModVars)(JNIEnv *env, jobject obj, jobject modVars) {
    ...
    // LOG_DEBUG("getModVars() called - tid: %d, initialized: %d", tid, state.isInitialized());
    ...
    // LOG_DEBUG("getModVars() - attempting lock - tid: %d", tid);
    std::unique_lock<std::mutex> lock = state.lock();
    // LOG_DEBUG("getModVars() - acquired lock - tid: %d", tid);
    ...
    // LOG_DEBUG("getModVars() completed - tid: %d", tid);
}
...
JNIEXPORT jbyteArray JNICALL JNI_FUNCTION(getComment)(JNIEnv *env, jobject obj) {
    ...
    env->SetByteArrayRegion(byteArray, 0, static_cast<jsize>(length),
                            reinterpret_cast<const jbyte *>(mi.comment));
    return byteArray;
}
...
JNI_FUNCTION(getPatternRow)(JNIEnv *env, jobject obj, jint pat, jint row, jbyteArray rowNotes,
                            jbyteArray rowInstruments, jbyteArray rowFxType, jbyteArray rowFxParm) {
    XmpPlayerState &state = XmpPlayerState::instance();
